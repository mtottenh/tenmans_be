...
def test_team():
    """Create a test team"""
    return Team(id=str(uuid.uuid4()), name="Test Team", tag="TEST", status="active")
...
async def test_submit_player_availability(
    availability_service, test_player, test_tournament, mock_session
):
    ...
    availability = await availability_service.submit_player_availability(
        player=test_player, availability_data=availability_data, session=mock_session
    )
...
async def test_submit_duplicate_availability(
    ...
    # Execute and assert
    with pytest.raises(
        AvailabilityServiceError, match="already submitted availability"
    ):
...
async def test_update_player_availability(
    availability_service, test_player, mock_session
):
...
async def test_delete_player_availability(
    ...
    await availability_service.delete_player_availability(
        availability_id=availability_id, player=test_player, session=mock_session
    )
...
async def test_get_player_availability_for_tournament(
    ...
    availabilities = [
        PlayerAvailability(
            ...
            end_time=datetime.now(timezone.utc) + timedelta(days=i, hours=4),
            availability_type=AvailabilityType.AVAILABLE,
        )
        for i in range(3)
    ]
    ...
    result = await availability_service.get_player_availability_for_tournament(
        player_id=test_player.id, tournament_id=test_tournament.id, session=mock_session
    )
...
async def test_calculate_team_availability(
    availability_service, test_team, test_tournament, test_roster, mock_session
):
    ...
    mock_scalars.side_effect = [
        ...
        *[
            Mock(all=Mock(return_value=[avail])) for avail in availabilities
        ],  # Subsequent calls for availabilities
    ]
    mock_session.execute.return_value.scalars = mock_scalars

    # Execute
    team_availabilities = await availability_service.calculate_team_availability(
        team=test_team, tournament=test_tournament, session=mock_session
    )
...
async def test_find_conflicts_for_schedule(
    availability_service, test_tournament, test_team, mock_session
):
...
async def test_suggest_optimal_schedules(
    ...
    for day_offset in range(7):
        start_time = datetime.now(timezone.utc) + timedelta(days=day_offset, hours=18)
...
@pytest.mark.asyncio
async def test_mark_availability_as_used(availability_service, mock_session):
    ...
    updated = await availability_service.mark_availability_as_used(
        availability_id=availability_id, session=mock_session
    )
...
@pytest.mark.asyncio
async def test_cleanup_old_availabilities(availability_service, mock_session):
    ...
    deleted_count = await availability_service.cleanup_old_availabilities(
        older_than_days=30, session=mock_session
    )
