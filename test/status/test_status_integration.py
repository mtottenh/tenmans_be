        ...
        **context,
        ...
        if new_status in ["DELETED", "DISBANDED"]:
            ...
            if hasattr(entity, "metadata"):
                ...
                entity.metadata["cleanup_date"] = datetime.now(timezone.utc).isoformat()
    ...
    async def execute(
        ...
        **context,
        ...
        if type(entity).__name__ == "Team" and new_status == "DISBANDING":
            ...
            if active_matches > 0:
                raise ValueError(
                    f"Cannot disband team with {active_matches} active matches"
                )
...
class TeamDisbandValidator(TransitionValidator):
    ...
    async def validate(
        self, current_status: Any, new_status: Any, context: dict[str, Any]
    ) -> bool:
...
class TestStatusIntegration:
    ...
    async def enhanced_service(self):
        ...
        service.register_transition_pipeline(
            ...
            "DISBANDING",
            TransitionPipeline(
                [ValidationStep(), NotificationStep(), DataCleanupStep()]
            ),
        )
    ...
    async def test_audit_trail_for_transitions(
        ...
        enhanced_service.register_transition_pipeline(
            "MockEntity", "PUBLISHED", TransitionPipeline([NotificationStep()])
        )
        ...
        async with AuditContext(session, entity_id=entity.id) as audit_context:
            ...
            notification_events = [
                e
                for e in audit_context.child_events
    ...
    async def test_failed_transition_rollback(
        ...
        enhanced_service.register_transition_pipeline(
            "FailEntity", "FAILED", TransitionPipeline([FailingStep()])
        )
