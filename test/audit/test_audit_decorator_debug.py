...
async def create_simple_table(session: AsyncSession):
    ...
    await session.execute(
        text(f"DROP TABLE IF EXISTS {SimpleTestEntity.__tablename__} CASCADE")
    )
    await session.execute(
        text(
            f"""
...
async def test_audit_decorator_flow(
    session: AsyncSession, system_user: Player, create_simple_table
):
    ...
    if new_events == 0:
        ...
        async with AuditContext(session, entity_id=entity.id) as ctx:
            ...
            await ctx.create_root_event(
                AuditEventType.UPDATE, "TestEntity", system_user, "Manual test"
            )
