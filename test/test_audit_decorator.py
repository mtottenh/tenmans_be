...
class TestEntityService:
    ...
    async def update_entity_with_param(
        ...
        target_entity.updated_at = datetime.now()
...
class TestAuditDecorator:
    ...
    async def test_update_with_entity_param(
        ...
        audit_events = await session.execute(
            AuditEvent.__table__.select().where(
                AuditEvent.entity_id == test_entity.id
            )
        )
        events = audit_events.fetchall()
        assert len(events) > 0
    ...
    async def test_entity_param_not_found_error(
        ...
        with pytest.raises(ValueError, match="Entity parameter 'nonexistent_param' not found"):
    ...
    async def test_entity_param_is_none(
        ...
        with pytest.raises(ValueError, match="Entity parameter 'target_entity' not found or is None"):
