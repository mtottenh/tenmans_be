...
async def test_authenticate_player_valid_credentials(
    ...
    # Verify method calls
    mock_identity_service.get_player_by_email.assert_called_once_with(
...
async def test_authenticate_player_invalid_email(
    auth_service, mock_identity_service, mock_session
):
...
async def test_authenticate_player_invalid_password(
    auth_service, mock_identity_service, test_player, mock_session
):
...
async def test_refresh_access_token_invalid_token_type(
    auth_service, mock_token_service, mock_session
):
...
async def test_check_player_permission(
    auth_service, mock_permission_service, test_player, mock_session
):
...
async def test_update_last_login(
    ...
    mock_identity_service.update_last_login.assert_called_once_with(
        test_player, mock_session
    )


@pytest.mark.asyncio
async def test_get_player_auth_info(
    auth_service, mock_permission_service, mock_role_service, test_player, mock_session
):
    ...
    roles = [Role(id="role1", name="player"), Role(id="role2", name="captain")]
