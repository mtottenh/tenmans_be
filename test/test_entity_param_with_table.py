...
async def ensure_test_table(session: AsyncSession):
    ...
    await session.execute(text(f"DROP TABLE IF EXISTS {TestEntityWithTable.__tablename__} CASCADE"))
    engine = session.bind
...
async def test_entity_param_with_table(
    ...
    assert len(second_events) >= 1, "entity_param='second' should audit the second entity"
