...
async def ensure_minimal_table(session: AsyncSession):
    ...
    await session.execute(text(f"DROP TABLE IF EXISTS {MinimalTestEntity.__tablename__} CASCADE"))
...
class TestEntityParamMinimal:
    ...
    async def test_update_with_audit(
        ...
        # Update entity
        updated = await service.update_entity(entity, "Updated Name", system_user, session)
        await session.commit()
    ...
    async def test_entity_param_selection(
        ...
        assert len(final_second_events) >= 1, "entity_param='second' should audit the second entity"
