...
def test_get_expiry_time(state_service):
    ...
    assert state_service.get_expiry_time(StateType.PASSWORD_RESET) == timedelta(
        hours=24
    )
    assert state_service.get_expiry_time(StateType.EMAIL_VERIFICATION) == timedelta(
        hours=48
    )
    assert state_service.get_expiry_time(StateType.FILE_UPLOAD) == timedelta(minutes=30)
    assert state_service.get_expiry_time(StateType.FILE_UPLOAD_RESULT) == timedelta(
        minutes=50
    )
...
async def test_set_state(state_service, test_state_data):
    ...
    await state_service.set_state(
        state_type=state_type, state_id=state_id, data=test_state_data
    )
...
async def test_set_state_with_custom_ttl(state_service, test_state_data):
    ...
    # Execute
    await state_service.set_state(
        state_type=state_type, state_id=state_id, data=test_state_data, ttl=custom_ttl
    )
...
async def test_get_state_raw(state_service):
    ...
    stored_state = State(type=state_type, data=json.dumps(state_data))

    state_service.redis.get.return_value = stored_state.model_dump_json()

    # Execute
    result = await state_service.get_state(state_type=state_type, state_id=state_id)
...
async def test_delete_state(state_service):
    ...
    # Execute
    result = await state_service.delete_state(state_type=state_type, state_id=state_id)

    # Assert
    assert result is True
    state_service.redis.delete.assert_called_once_with(f"state:{state_type}:{state_id}")
...
async def test_delete_state_not_found(state_service):
    ...
    result = await state_service.delete_state(
        state_type=StateType.GENERAL, state_id="non-existent"
    )
...
async def test_exists(state_service):
    ...
    # Execute
    result = await state_service.exists(state_type=state_type, state_id=state_id)

    # Assert
    assert result is True
    state_service.redis.exists.assert_called_once_with(f"state:{state_type}:{state_id}")
...
async def test_extend_ttl(state_service):
    ...
    result = await state_service.extend_ttl(
        state_type=state_type, state_id=state_id, additional_time=additional_time
    )

    # Assert
    assert result is True
    state_service.redis.expire.assert_called_once_with(
        f"state:{state_type}:{state_id}", additional_time.total_seconds()
    )
...
async def test_update_state(state_service, test_state_data):
    ...
    # Mock existing state
    existing_state = State(type=state_type, data=test_state_data.model_dump_json())
...
async def test_bulk_set(state_service):
    ...
    # Setup
    states = [(StateType.GENERAL, f"id_{i}", {"value": i}) for i in range(3)]
...
async def test_bulk_get(state_service):
    ...
    for i, state_id in enumerate(state_ids):
        state = State(type=state_type, data=json.dumps({"id": state_id, "value": i}))
    ...
    # Execute
    results = await state_service.bulk_get(state_type=state_type, state_ids=state_ids)
...
async def test_scan_keys(state_service):
    ...
    # Mock scan response
    state_service.redis.scan.return_value = (
        0,
        [f"state:{state_type}:test_1", f"state:{state_type}:test_2"],
    )

    # Execute
    keys = await state_service.scan_keys(state_type=state_type, pattern=pattern)
...
async def test_get_with_metadata(state_service, test_state_data):
    ...
    metadata = {"created_at": datetime.now(timezone.utc).isoformat(), "created_by": "test_user"}

    stored_state = State(
        type=state_type, data=test_state_data.model_dump_json(), metadata=metadata
    )
...
async def test_atomic_update(state_service, test_state_data):
    ...
    await state_service.atomic_update(
...
async def test_create_ephemeral_token(state_service):
    ...
    token = await state_service.create_ephemeral_token(
        user_id=user_id, ttl=timedelta(minutes=5)
    )
