...
async def test_tournament_with_team(
    ...
    tournament_create = TournamentCreate(
        ...
        registration_end=datetime.now(timezone.utc) + timedelta(days=1),
        ...
        max_team_size=7,
        format_config={"teams_per_group": 4, "match_format": "bo3"},
    )

    tournament = await tournament_service.create_tournament(
        tournament_data=tournament_create, actor=system_user, session=session
    )
    ...
    await tournament_service.request_registration(
        registration=registration_request, actor=captain, session=session
    )
...
async def test_captain_can_disband_team(
    session: AsyncSession, test_team_with_members: tuple[Team, list[Player]]
):
    ...
    await team_service.disband_team(
        team=team, reason="Testing team disbanding", actor=captain, session=session
    )
    ...
    all_rosters = await roster_service.get_team_roster(
        team, season, session, include_all=True
    )
...
async def test_admin_can_disband_team(
    ...
    # Admin disbands team
    await team_service.disband_team(
        team=team, reason="Admin disbanding test", actor=admin, session=session
    )
...
async def test_disbanding_affects_tournament_registration(
    ...
    # Disband team
    await team_service.disband_team(
        team=team, reason="Testing tournament effects", actor=captain, session=session
    )
...
async def test_disbanding_with_active_matches(
    ...
    # Disband team
    await team_service.disband_team(
        team=team, reason="Testing match effects", actor=captain, session=session
    )
...
async def test_prevent_rejoining_disbanded_team(
    ...
    # Disband team
    await team_service.disband_team(
        team=team, reason="Testing join prevention", actor=captain, session=session
    )
