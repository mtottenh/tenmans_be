...
@pytest.mark.asyncio
async def test_reschedule_fixture(
    fixture_service,
    test_fixture,
    mock_session
):
    ...
    # Setup
    new_time = datetime.now() + timedelta(days=14)
...
async def test_reschedule_completed_fixture(
    ...
    reschedule_data = FixtureReschedule(
        new_scheduled_at=datetime.now() + timedelta(days=14),
...
@pytest.mark.asyncio
async def test_forfeit_fixture(
    fixture_service,
    test_fixture,
    test_teams,
    mock_session
    ...
    forfeit_data = FixtureForfeit(
        forfeit_team_id=test_teams[0].id,
        reason="Team didn't show up"
    )
...
async def test_get_team_fixtures(
    ...
    # Assert
    assert len(result) == 1
    assert result[0].team_1 == team_id or result[0].team_2 == team_id


@pytest.mark.asyncio
async def test_update_fixture_status(
    fixture_service,
    test_fixture,
    mock_session
):
