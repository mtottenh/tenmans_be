...
from competitions.base_schemas import FixtureStatus as BaseFixtureStatus
...
@pytest.fixture
def round_service(mock_audit_service, mock_status_transition_service, mock_round_winner_service):
...
def test_teams():
    """Create test teams"""
    return [
        {"id": str(uuid.uuid4()), "name": f"Team {i}"} 
        for i in range(4)
    ]
...
async def test_start_round(
    ...
    test_round_started.started_at = datetime.now()
...
async def test_complete_round(
    ...
    test_round_completed.completed_at = datetime.now()
...
async def test_get_round_standings(
    ...
    with patch.object(round_service, 'calculate_round_standings', return_value=standings):
        # Execute
...
@pytest.mark.asyncio
async def test_update_round_dates(
    round_service,
    test_round,
    test_player,
    mock_session
