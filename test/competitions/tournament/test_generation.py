...
class TestTournamentGeneration:
    ...
    async def test_regular_tournament_generation(
        ...
        for round in rounds:
            ...
            team_ids = {team.id for team in regular_tournament_setup["teams"]}
    ...
    async def test_knockout_tournament_generation(
        ...
        # Verify round structure
        assert (
            len(rounds) == len(knockout_tournament_setup["teams"]).bit_length() - 1
        )  # Log2 ceiling
    ...
    @pytest.mark.parametrize(
        "team_count,min_teams,should_raise",
        [
    ...
    async def test_team_count_validation(
        ...
        await test_data_builder.generate_players(
            team_count * 5
        )  # Enough players for teams
    ...
    async def test_round_date_generation(
        ...
        for i in range(len(rounds) - 1):
            ...
            assert (
                rounds[i + 1].start_date - rounds[i].start_date
            ).days == 7  # One week between rounds
