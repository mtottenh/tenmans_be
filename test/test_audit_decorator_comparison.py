...
class TestEntityParamComparison:
    ...
    async def test_default_behavior_audits_wrong_entity(
        ...
        await service.register_team_default(
            team,
            tournament,
            system_user,
            session
        ...
        tournament_events = await session.execute(
            AuditEvent.__table__.select().where(
                AuditEvent.entity_id == tournament.id
            )
        )
    ...
    async def test_entity_param_audits_correct_entity(
        ...
        await service.register_team_with_param(
            team,
            tournament,
            system_user,
            session
        ...
        assert len(tournament_audit_events) > 0
        latest_tournament_event = max(tournament_audit_events, key=lambda e: e.timestamp)
