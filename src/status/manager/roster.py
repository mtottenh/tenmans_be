...
class RosterTransitionPermissionValidator(TransitionValidator):
    ...
    async def _validate_player_transition(
        ...
        if current_status == RosterStatus.ACTIVE and new_status in [
    ...
    async def _validate_captain_transition(
        self, current_status: RosterStatus, new_status: RosterStatus
    ) -> bool:
...
class RosterReasonValidator(TransitionValidator):
    ...
    async def validate(
        self,
        current_status: RosterStatus,  # noqa: ARG002
        new_status: RosterStatus,  # noqa: ARG002
        context: dict[str, Any],
    ) -> bool:
...
def initialize_roster_status_manager() -> StatusTransitionManager:
    ...
    manager = StatusTransitionManager(status_enum=RosterStatus, entity_type="Roster")
