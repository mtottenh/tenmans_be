...
class HasValidReasonValidator(TransitionValidator):
    ...
    async def validate(
        self,
        current_status: TeamCaptainStatus,  # noqa: ARG002
        new_status: TeamCaptainStatus,  # noqa: ARG002
        context: dict[str, Any],
    ) -> bool:
        reason = context.get("reason")
        if not reason or not reason.strip():
            raise TransitionError(
                "A reason must be provided for captain status changes"
            )
        return True
...
class TeamPermissionValidator(TransitionValidator):
    ...
    async def validate(
        ...
        if (
            current_status == TeamCaptainStatus.PENDING
            and new_status == TeamCaptainStatus.ACTIVE
            and context.get("is_initial_captain")
        ):
            return True

        # Regular permission checks...
        has_admin = await permission_service.verify_permissions(
            actor, ["manage_teams"], None, session
        )
        if has_admin:
            return True

        is_captain = await permission_service.verify_permissions(
            actor, ["manage_team"], PermissionScope(ScopeType.TEAM, team_id), session
        )
        if not is_captain:
...
def initialize_captain_status_manager() -> StatusTransitionManager:
    ...
    manager = StatusTransitionManager(
        status_enum=TeamCaptainStatus, entity_type="TeamCaptain"
    )

    # Common validators
    common_validators = [HasValidReasonValidator(), TeamPermissionValidator()]
