...
class HasValidReasonValidator(TransitionValidator):
    ...
    async def validate(
        self,
        current_status: JoinRequestStatus,  # noqa: ARG002
        new_status: JoinRequestStatus,  # noqa: ARG002
        context: dict[str, Any],
    ) -> bool:
        reason = context.get("reason")
        if not reason or not reason.strip():
            raise TransitionError(
                "A reason must be provided for join request status changes"
            )
        return True
...
class TeamCaptainValidator(TransitionValidator):
    ...
    async def validate(
        ...
        if not is_captain:
            raise TransitionError(
                "Only team captains can approve or reject join requests"
            )
        return True
...
class RequesterOnlyValidator(TransitionValidator):
    ...
    async def validate(
        ...
        if actor.id != join_request.player_id:
            raise TransitionError(
                "Only the requesting player can cancel their join request"
            )
        return True
...
def initialize_join_request_manager() -> StatusTransitionManager:
    ...
    manager = StatusTransitionManager(
        status_enum=JoinRequestStatus, entity_type="TeamJoinRequest"
    )
    ...
    manager.add_rule(
        StatusTransitionRule(
            ...
            validators=[
                *common_validators,
                NoExistingTeamValidator(),
                NoDuplicateRequestValidator(),
            ],
        )
    )
