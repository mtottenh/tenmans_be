...
class StatusTransitionManager(Generic[T]):
    ...
    def __init__(self, status_enum: type[StrEnum], entity_type: str):
    ...
    async def validate_transition(
        ...
        for rule in self.rules:
            ...
            if (
                rule.from_status is None or current_status in rule.from_status
            ) and new_status in rule.to_status:
                valid_rule = rule
                break
        ...
        if actor and valid_rule.required_permissions:
            ...
            has_permission = await permission_service.verify_permissions(
                actor, valid_rule.required_permissions, scope, session
            )
