    ...
    async def execute(
        ...
        **context,
        ...
        for captain in captains:
            LOG.info(
                f"Setting captain {captain.player_id} to TEMPORARY due to team suspension"
            )
    ...
    async def execute(
        ...
        **context,
        ...
        for fixture in fixtures:
            ...
            fixture.reschedule_reason = (
                f"Team {entity.name} suspended: {context.get('reason')}"
            )
...
class TeamCaptainReactivateStep(TransitionStep):
    ...
    async def execute(
        ...
        **context,
        ...
        if (
            new_status != TeamStatus.ACTIVE.value
            or old_status != TeamStatus.SUSPENDED.value
        ):
        ...
        for captain in captains:
            LOG.info(
                f"Restoring captain {captain.player_id} to ACTIVE due to team reactivation"
            )
