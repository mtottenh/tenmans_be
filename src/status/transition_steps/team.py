    ...
    async def execute(
        ...
        **context,
        ...
        stmt = select(Roster).where(
            Roster.team_id == entity.id, Roster.status == RosterStatus.ACTIVE
        )
        result = await session.execute(stmt)
        ...
        for roster in rosters:
            LOG.info(
                f"Setting roster entry {roster.player_id} to PAST due to team disbandment"
            )
    ...
    async def execute(
        ...
        **context,
        ...
        for captain in captains:
            LOG.info(
                f"Setting captain {captain.player_id} to DISBANDED due to team disbandment"
            )
    ...
    async def execute(
        ...
        **context,
        ...
        for registration in registrations:
            LOG.info(
                f"Withdrawing team from tournament {registration.tournament_id} due to disbandment"
            )
