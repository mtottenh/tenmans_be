...
class MapPoolVotingCompleteStep(TransitionStep):
    ...
    async def execute(
        ...
        **context,
        ...
        stmt = (
            select(MapPoolMap)
            .where(MapPoolMap.pool_id == entity.id)
        ...
        LOG.info(
            f"Finalizing map pool {entity.id} with {len(selected_maps)} selected maps"
        )

        # Remove unselected maps
        for pool_map in unselected_maps:
            LOG.info(
                f"Removing unselected map {pool_map.map_id} with {pool_map.vote_count} votes"
            )
...
class MapPoolNotificationStep(TransitionStep):
    ...
    async def execute(
        ...
        **context,
        ...
        LOG.info(
            f"Map pool {entity.id} finalized for tournament {entity.tournament_id}"
        )
