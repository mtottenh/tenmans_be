...
from auth.models import Player, Role, PlayerRole
    ...
    async def execute(
        ...
        **context
        ...
        for roster in rosters:
            LOG.info(f"Setting roster entry for team {roster.team_id} to REMOVED due to player ban")
    ...
    async def execute(
        ...
        **context
        ...
        for captain in captains:
            LOG.info(f"Setting captain entry for team {captain.team_id} to REMOVED due to player ban")
...
class PlayerFixtureSubstituteStep(TransitionStep):
    ...
    async def execute(
        ...
        **context
        ...
        stmt = select(MatchPlayer).join(
            Fixture,
            MatchPlayer.fixture_id == Fixture.id
        ).where(
        ...
        for match_player in match_players:
            LOG.info(f"Removing player from fixture {match_player.fixture_id} due to ban")
...
class PlayerUnbanRestoreStep(TransitionStep):
    ...
    async def execute(
        ...
        **context
        ...
        if old_status != PlayerStatus.BANNED.value or new_status != PlayerStatus.ACTIVE.value:
