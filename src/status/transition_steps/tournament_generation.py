    ...
    async def execute(
        ...
        **context,
        ...
        try:
            ...
            LOG.error(f"Tournament validation failed: {e!s}")
            raise
    ...
    async def execute(
        ...
        **context,
        ...
        if old_status == TournamentState.REGISTRATION_OPEN.value:
            ...
            entity.registration_end = datetime.now(timezone.utc)
            session.add(entity)
    ...
    async def execute(
        ...
        **context,
        ...
        if new_status == TournamentState.IN_PROGRESS.value:
            ...
            entity.actual_start_date = datetime.now(timezone.utc)

            # Activate first round
            stmt = select(Round).where(
                Round.tournament_id == entity.id, Round.round_number == 1
            )
            result = await session.execute(stmt)
