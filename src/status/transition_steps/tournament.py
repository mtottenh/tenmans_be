    ...
    async def execute(
        ...
        **context,
        ...
        for registration in registrations:
            LOG.info(
                f"Withdrawing team {registration.team_id} from tournament due to cancellation"
            )
            ...
            registration.withdrawal_reason = (
                f"Tournament cancelled: {context.get('reason')}"
            )
    ...
    async def execute(
        ...
        **context,
        ...
        stmt = select(Round).where(
            Round.tournament_id == entity.id, Round.status != "completed"
        )
        result = await session.execute(stmt)
        ...
        for tournament_round in rounds:
            LOG.info(
                f"Completing round {tournament_round.round_number} due to tournament completion"
            )
            tournament_round.status = "completed"
            tournament_round.updated_at = datetime.now(timezone.utc)
