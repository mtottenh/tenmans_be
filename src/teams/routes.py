...
from teams.base_schemas import TeamCaptainStatus, TeamCreateRequest, TeamUpdate
...
from .dependencies import require_team_captain_by_id
...
async def create_team(
    ...
    try:
        ...
        new_team = await team_service.get_team_by_id(
            team_id=new_team.id, session=session
        )
        return new_team
...
async def disband_team(
    ...
    return await team_service.disband_team(
        team, "REASON", actor=current_player, session=session
    )
...
async def get_team_logo_by_id(
    ...
    if not team:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Unknown team ID"
        )
    if not team.logo:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Team has not yet uploaded a logo",
        )
    if not Path(team.logo).exists():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def get_team_logo_by_name(
    ...
    team = await team_service.get_team_by_name(name, session)
    if not team or not team.logo or not Path(team.logo).exists():
        raise HTTPException(
...
@team_router.get("/name/{team_name}/captains", response_model=list[PlayerPublic])
...
@team_router.get("/id/{team_id}/captains", response_model=list[PlayerPublic])
async def get_team_captains_by_id(
    team_id: str,
    session: AsyncSession = Depends(get_session),
    current_player: Player = Depends(get_current_player),
):
...
async def add_team_captain(
    ...
    try:
        ...
        return await team_service.create_captain(
            team=team, player=new_captain, actor=current_player, session=session
        )
...
async def remove_team_captain(
    ...
    try:
        ...
        captain: TeamCaptain = await team_service.get_captain(team, player, session)
        if not captain:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Player {player_id} is not a captain of {team_id}",
            )

        if captain.status in [TeamCaptainStatus.ACTIVE, TeamCaptainStatus.TEMPORARY]:
        ...
        await team_service.remove_captain(
            captain, actor=current_player, session=session
        )
...
async def remove_player_from_team(
    ...
    await team_service.remove_player_from_team_roster(
        team_id, player_id, current_season.id, actor=current_player, session=session
    )


@team_router.patch("/id/{team_id}", dependencies=[Depends(require_team_captain_by_id)])
async def update_team_settings(
    ...
    if not team:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"No team with id {team_id}"
        )
    final_path = None
    if team_update_details.logo_token_id:
        upload_result = await upload_service.get_upload_result(
            team_update_details.logo_token_id
        )
        if not upload_result:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
