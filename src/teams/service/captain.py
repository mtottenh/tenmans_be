...
class CaptainService:
    def __init__(
        ...
        self.permission_service = permission_service or create_permission_service(
            self.audit_service
        )
        ...
        self.status_transition_service = (
            ...
            or create_status_transition_service(
                self.audit_service, self.permission_service
            )
        )
        ...
        self.status_transition_service.register_transition_manager(
            "TeamCaptain", captain_manager
        )

    def _captain_audit_details(self, captain: TeamCaptain, context: dict) -> dict:  # noqa: ARG002
        """Extracts audit details from a captain operation"""
        return {
            "team_id": str(captain.team_id),
            "player_id": str(captain.player_id),
            "status": captain.status,
            "created_at": captain.created_at.isoformat()
            if captain.created_at
            else None,
        }
    ...
    async def get_active_captains(
        self, team: Team, session: AsyncSession
    ) -> list[TeamCaptain]:
    ...
    async def remove_captain(
        ...
        team_captain_role = await self.role_service.get_role_by_name(
            "team_captain", session
        )
    ...
    async def get_team_captains(
        self, team_name: str, session: AsyncSession
    ) -> list[Player]:
    ...
    async def get_team_captains_by_team_id(
        ...
        stmt = (
            ...
            .where(
                ...
                TeamCaptain.status.in_(
                    [
                        TeamCaptainStatus.ACTIVE,
                        TeamCaptainStatus.TEMPORARY,
                        TeamCaptainStatus.PENDING,
                    ]
                ),
            )
            .options(selectinload(Player.roles).selectinload(Role.permissions))
...
def create_captain_service(
    ...
    return CaptainService(
        audit_service, permission_service, role_service, status_transition_service
    )
