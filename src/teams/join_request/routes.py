...
@team_join_request_router.post("/", status_code=status.HTTP_201_CREATED)
...
async def get_team_join_requests_id(
    ...
    requests = await join_request_service.get_team_requests(
        team=team, session=session, include_resolved=include_resolved
    )

    # Calculate stats
    pending_count = sum(1 for r in requests if r.status == JoinRequestStatus.PENDING)
    return JoinRequestList(
        total=len(requests), pending_count=pending_count, requests=requests
    )


# TODO - implement get_team_req_with_req_id
@team_join_request_router.get("/id/{request_id}", response_model=JoinRequestDetailed)
async def get_join_request(
    ...
    request = await join_request_service.get_team_req_with_req_id(
        team_id, request_id, session
    )
    if not request:
        raise HTTPException(
    ...
    is_captain = await team_service.player_is_team_captain(
        current_player, request.team, session
    )
...
async def approve_join_request(
    ...
    try:
        request = await join_request_service.get_pending_team_request_by_id(
            team_id, request_id, session
        )
        if request is None:
            raise HTTPException(
...
@team_join_request_router.patch(
    "/id/{request_id}/cancel", response_model=JoinRequestDetailed
)
...
async def get_team_request_stats(
    ...
    requests = await join_request_service.get_team_requests(
        team=team, session=session, include_resolved=True
    )

    # Calculate stats
    total = len(requests)
    stats = {
        status: sum(1 for r in requests if r.status == status)
...
global_join_request_router = APIRouter(prefix="/join-requests")


@global_join_request_router.get("/me", response_model=JoinRequestList)
async def get_my_join_requests(
    ...
    requests = await join_request_service.get_player_requests(
        player=current_player, session=session, include_resolved=include_resolved
    )
