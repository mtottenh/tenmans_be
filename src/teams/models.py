...
# Perhaps instead I should create a settings table with key/value pairs
# And have a setting called 'active_season'
...
class Team(SQLModel, table=True):
    ...
    name: str = Field(unique=True)
    player_links: list[Roster] = Relationship(back_populates="team",  sa_relationship_kwargs={"lazy":"selectin"})
    created_at: datetime = Field(sa_column=Column(sl.TIMESTAMP, default=datetime.now))
    update_at: datetime = Field(sa_column=Column(sl.TIMESTAMP, default=datetime.now))
