...
from competitions.map_pool.models import MapPoolMap, MapPoolSelectionType, MapPoolStatus, MapPoolVote
from competitions.map_pool.schemas import MapPoolCreate, MapPoolVoteRequest, MapPoolVoteResponse
...
class TournamentService:
    ...
    def _map_pool_audit_details(self, map_pool: TournamentMapPool, context: Dict) -> dict:
        ...
        return {
            ...
            "voting_start": map_pool.voting_start.isoformat() if map_pool.voting_start else None,
            "voting_end": map_pool.voting_end.isoformat() if map_pool.voting_end else None,
            "finalized_at": map_pool.finalized_at.isoformat() if map_pool.finalized_at else None
        }

    def _map_vote_audit_details(self, vote: MapPoolVote, context: Dict) -> dict:
    ...
    async def create_tournament(
        ...
        try:
            ...
            for field in ['registration_start', 'registration_end', 'scheduled_start_date', 
                        'scheduled_end_date', 'late_registration_end']:
                if field in tournament_dict and tournament_dict[field]:
                    tournament_dict[field] = tournament_dict[field].astimezone(timezone.utc).replace(tzinfo=None)
            ...
            if (tournament_data.map_selection_method == MapSelectionMethod.ADMIN_ASSIGNED and 
                tournament_data.initial_map_pool):
    ...
    async def link_tournaments(
        ...
        if target.type != TournamentType.KNOCKOUT:
            raise TournamentServiceError("Target tournament must be a knockout tournament")
    ...
    async def generate_tournament_structure(
        ...
        try:
            ...
            if tournament.type == TournamentType.REGULAR and tournament.league_format != LeagueFormat.SWISS:
    ...
    async def get_map_pool(
        ...
        stmt = select(TournamentMapPool).where(
            TournamentMapPool.tournament_id == tournament_id
        ).options(
    ...
    async def create_map_pool(
        ...
        if map_pool_data.selection_type == MapPoolSelectionType.ADMIN_DEFINED:
            ...
            voting_end = voting_start + timedelta(days=map_pool_data.voting_duration_days)
    ...
    async def vote_for_maps(
        ...
        is_captain = await self.team_service.player_is_team_captain(actor, team, session)
        if not is_captain:
            raise TournamentServiceError("Only team captains can vote for maps")
        
        # Check existing votes
        stmt = select(MapPoolVote).where(
            MapPoolVote.pool_id == map_pool.id,
            MapPoolVote.team_id == team.id
        )
        result = await session.execute(stmt)
        existing_votes = result.scalars().all()
        
        if len(existing_votes) + len(vote_request.map_ids) > map_pool.votes_per_team:
            raise TournamentServiceError(f"Exceeds vote limit of {map_pool.votes_per_team}")
        ...
        for map_id in vote_request.map_ids:
            ...
            stmt = select(MapPoolMap).where(
                MapPoolMap.pool_id == map_pool.id,
        ...
        return MapPoolVoteResponse(
            ...
            remaining_votes=map_pool.votes_per_team - len(existing_votes) - len(vote_request.map_ids)
        )
    ...
    async def finalize_map_pool(
        ...
        selected_maps = all_pool_maps[:map_pool.maps_to_select]
    ...
    async def get_tournament_with_stats(
        ...
        matches_completed = len([f for f in fixtures if f.status == FixtureStatus.COMPLETED])
        matches_remaining = len([f for f in fixtures if f.status == FixtureStatus.SCHEDULED])
        ...
        active_registrations = len([
            reg for reg in tournament.registrations 
