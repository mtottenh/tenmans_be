...
class TournamentValidator:
    ...
    def _validate_knockout_config(tournament: Tournament):
        ...
        if config["seeding_type"] not in valid_seeding:
            raise ValidationError(
                f"Invalid seeding type. Must be one of: {valid_seeding}"
            )
    ...
    def validate_tournament_dates(tournament: Tournament):
        ...
        if (tournament.allow_late_registration and tournament.late_registration_end and
            tournament.late_registration_end >= tournament.scheduled_start_date):
                raise ValidationError(
                    "Late registration must end before tournament starts"
                )
    ...
    def validate_teams(teams: list[Team], tournament: Tournament):
        ...
        for team in teams:
            active_roster_count = sum(
                1 for roster in team.rosters if roster.status == "ACTIVE"
            )
    ...
    def validate_round_dates(rounds: list[Round], tournament: Tournament):
        ...
        for i in range(len(rounds) - 1):
            if rounds[i].end_date > rounds[i + 1].start_date:
                raise ValidationError(f"Round {i + 1} ends after round {i + 2} starts")
