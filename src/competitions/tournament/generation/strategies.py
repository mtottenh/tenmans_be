...
class SeedingStrategy(ABC):
    ...
    async def seed_teams(
        self, teams: list[Team], tournament: Tournament, session
    ) -> list[Team]:
...
class RandomSeedingStrategy(SeedingStrategy):
    ...
    async def seed_teams(
        self, teams: list[Team], tournament: Tournament, session  # noqa: ARG002
    ) -> list[Team]:
        shuffled = teams.copy()
...
class EloSeedingStrategy(SeedingStrategy):
    ...
    async def _calculate_team_elo(self, team: Team, session) -> float:  # noqa: ARG002
...
class StandingsSeedingStrategy(SeedingStrategy):
    ...
    async def seed_teams(
        ...
        source_standings = await standings_calc.calculate_standings(
            tournament_id=self.source_tournament_id, session=session
        )

        # Create ranking map
        team_ranks = {
            entry.team_id: idx for idx, entry in enumerate(source_standings.teams)
        }
...
class TournamentGenerationStrategy(ABC):
    ...
    async def generate_rounds(
        self, tournament: Tournament, teams: list[Team], session
    ) -> list[Round]:
    ...
    async def generate_fixtures(
        self, tournament: Tournament, round: Round, teams: list[Team], session
    ) -> list[Fixture]:
...
class RoundRobinStrategy(TournamentGenerationStrategy):
    ...
    async def generate_rounds(
        self, tournament: Tournament, teams: list[Team], session  # noqa: ARG002
    ) -> list[Round]:
        ...
        for round_num in range(num_rounds):
            round = Round(
                ...
                best_of=self._get_match_format_games(
                    tournament.format_config["match_format"]
                ),
                start_date=round_start
                + timedelta(days=round_num * days_between_rounds),
                end_date=round_start
                + timedelta(days=(round_num + 1) * days_between_rounds),
                status="pending",
            )
            rounds.append(round)
    ...
    async def generate_fixtures(
        ...
        for i in range(half):
            ...
            if team_1 is not None and team_2 is not None:
                ...
                if (
                    tournament.league_format == "double_round_robin"
                    and round.round_number > (n - 1)
                ):
    ...
    async def generate_fixtures(
        ...
        if round.round_number == 1:
            ...
            for i in range(0, target_size, 2):
                if i < num_byes * 2 and i // 2 < len(seeded_teams):
...
class SwissSystemStrategy(TournamentGenerationStrategy):
    ...
    async def generate_rounds(
        ...
        num_teams = len(teams)
        num_rounds = tournament.format_config.get(
            "num_rounds", (num_teams - 1).bit_length() + 1
        )
    ...
    async def generate_fixtures(
        ...
        if round.round_number == 1:
            ...
            return await self._generate_swiss_pairings(
                tournament, round, teams, session
            )

    def _generate_first_round_pairings(
        self, tournament: Tournament, round: Round, teams: list[Team]
    ) -> list[Fixture]:
        ...
        for score in sorted_scores:
            ...
            while len(available_teams) >= 2:
                ...
                for candidate in available_teams:
                    if not self._have_played_before(
                        team_1.id, candidate.id, previous_matches
                    ):
                        team_2 = candidate
                        break
    ...
    async def _get_previous_matches(
        self, tournament_id: uuid.UUID, session
...
def get_generation_strategy(tournament: Tournament) -> TournamentGenerationStrategy:
    ...
    if tournament.type == TournamentType.REGULAR:
        ...
        raise ValueError(
            f"No generation strategy for tournament type: {tournament.type}"
        )
