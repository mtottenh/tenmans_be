...
from .schemas import TournamentStandings, TournamentTeam
...
class StandingsCalculator(ABC):
    ...
    def get_calculator(tournament_type: TournamentType) -> "StandingsCalculator":
        ...
        if not calculator:
            raise ValueError(
                f"No standings calculator for tournament type: {tournament_type}"
            )
...
class RegularStandingsCalculator(StandingsCalculator):
            ...
            if fixture.status in [FixtureStatus.COMPLETED, FixtureStatus.FORFEITED]:
                ...
                if team_1_id in team_stats and team_2_id in team_stats:
                    ...
                    if fixture.status == FixtureStatus.FORFEITED:
                        ...
                        winner_id = await self._determine_fixture_winner(
                            fixture, results
                        )
                    ...
                    if winner_id:
                        ...
                        team_stats[winner_id]["points"] += (
                            3  # Standard 3 points for a win
                        )
    ...
    async def _determine_fixture_winner(
        self, fixture: Fixture, results: list[Any]
    ) -> Optional[uuid.UUID]:
                ...
                if fixture.status in [FixtureStatus.COMPLETED, FixtureStatus.FORFEITED]:
                    ...
                    if team_1_id in team_stats and team_2_id in team_stats:
                        ...
                        if winner_id:
                            ...
                            # Update loser
                            loser_id = (
                                team_2_id if winner_id == team_1_id else team_1_id
                            )
                            team_stats[loser_id]["matches_lost"] += 1
                            team_stats[loser_id]["status"] = "eliminated"
                            team_stats[loser_id]["eliminated_round"] = (
                                round.round_number
                            )
        ...
        if final_round and final_round.status == "completed":
            ...
            if final_fixtures:
                ...
                if winner_id:
                    ...
                    # Runner-up
                    runner_up_id = (
                        final_fixture.team_2
                        if winner_id == final_fixture.team_1
                        else final_fixture.team_1
                    )
