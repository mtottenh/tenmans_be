...
class LinkedTournament(SQLModel, table=True):
    ...
    source_tournament_id: uuid.UUID = Field(sa_column=Column(ForeignKey("tournaments.id")))  # League
    target_tournament_id: uuid.UUID = Field(sa_column=Column(ForeignKey("tournaments.id")))  # Knockout
    ...
    source_tournament: "Tournament" = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[LinkedTournament.source_tournament_id]"}
    )
    target_tournament: "Tournament" = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[LinkedTournament.target_tournament_id]"}
    )