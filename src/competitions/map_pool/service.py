...
class MapPoolService:
    async def create_admin_map_pool(
        self, tournament_id: uuid.UUID, map_ids: list[uuid.UUID], session: AsyncSession
    ) -> TournamentMapPool:
        ...
        pool_maps = [
            MapPoolMap(pool_id=map_pool.id, map_id=map_id) for map_id in map_ids
        ]
    ...
    async def cast_team_vote(
        ...
        stmt = select(MapPoolVote).where(
            MapPoolVote.pool_id == pool_id, MapPoolVote.team_id == team_id
        )
        ...
        # Record the vote
        vote = MapPoolVote(pool_id=pool_id, team_id=team_id, map_id=map_id)
        session.add(vote)

        # Update vote count in map_pool_maps
        stmt = select(MapPoolMap).where(
            MapPoolMap.pool_id == pool_id, MapPoolMap.map_id == map_id
        )
        result = (await session.execute(stmt)).scalars()
    ...
    async def finalize_voting_pool(
        self, pool_id: uuid.UUID, session: AsyncSession
    ) -> TournamentMapPool:
        ...
        stmt = (
            select(MapPoolMap)
            .where(MapPoolMap.pool_id == pool_id)
    ...
    async def get_pool_status(self, pool_id: uuid.UUID, session: AsyncSession) -> dict:
