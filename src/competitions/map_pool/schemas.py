...
class MapPoolCreate(BaseModel):
    ...
    def validate_selection_type_fields(self) -> Self:
        ...
        elif (self.selection_type == MapPoolSelectionType.TEAM_VOTING and
              not all([self.voting_duration_days, self.maps_to_select, self.votes_per_team])):
                raise ValueError(
                    "voting_duration_days, maps_to_select, and votes_per_team required for voting pools"
                )
        return self
