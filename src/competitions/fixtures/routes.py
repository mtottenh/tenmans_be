...
async def get_upcoming_fixtures(
    ...
    fixtures = await fixture_service.get_upcoming_fixtures(
        season_id=current_season.id, days=days, session=session
    )

    # Count fixtures in different time periods
    now = datetime.now(timezone.utc)
    next_24h = sum(1 for f in fixtures if f.scheduled_at <= now + timedelta(days=1))
    next_week = sum(1 for f in fixtures if f.scheduled_at <= now + timedelta(days=7))

    return UpcomingFixturesResponse(
        items=fixtures, total=len(fixtures), next_24h=next_24h, next_week=next_week
    )


@fixture_router.get(
    "/", response_model=FixturePage, dependencies=[Depends(require_view_matches)]
)
...
@fixture_router.get("/id/{fixture_id}", response_model=FixtureDetailed)
...
async def create_fixture(
    ...
    try:
        return await fixture_service.create_fixture(
            fixture_data=fixture_data, actor=current_player, session=session
        )
...
async def start_fixture(
    ...
    try:
        ...
        return await fixture_service.start_fixture(
            fixture=fixture, actor=current_player, session=session
        )
    except FixtureServiceError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e)) from e
