...
class RoleService:
    ...
    def _role_audit_details(self, role: Role, context: dict) -> dict:  # noqa: ARG002
    ...
    def _player_role_audit_details(self, role: PlayerRole, context: dict) -> dict:  # noqa: ARG002
        return {
    ...
    def _id_extractor_player_role(self, role: Role) -> uuid:  # noqa: ARG002
        return uuid.uuid4()

    async def get_role(
        self, role_id: uuid.UUID, session: AsyncSession
    ) -> Optional[Role]:
        """Get a role by ID"""
    ...
    async def verify_role(
        self, player: Player, allowed_roles: list[str], session: AsyncSession
    ) -> bool:
    ...
    async def remove_role_from_player(
        ...
        return await self.delete_player_role(
            role_to_remove[0], actor=actor, session=session, audit_context=audit_context
        )
...
def create_role_service(
    permission_svc: Optional[PermissionService] = None,
) -> RoleService:
