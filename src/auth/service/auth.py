...
from auth.service.token import TokenService, create_token_service
...
class AuthService:
    ...
    async def authenticate_player(
        self, login_data: PlayerLogin, session: AsyncSession
    ) -> Optional[tuple[Player, TokenResponse]]:
        """Authenticate a player and return tokens if successful"""
        player = await self.identity_service.get_player_by_email(
            login_data.email, session
        )

        if not player or not self.identity_service.verify_password(
            login_data.password, player.password_hash
        ):
            return None
    ...
    async def create_player(
        ...
        player = await self.identity_service.create_player_with_email(
            player_data, actor=actor, session=session, audit_context=audit_context
        )
    ...
    async def create_steam_player(
        ...
        player = await self.identity_service.create_player_with_steam(
            steam_id, actor=actor, session=session, audit_context=audit_context
        )
    ...
    async def get_all_players_with_basic_team_info(self, *args, **kwargs):
        return await self.identity_service.get_all_players_with_basic_team_info(
            *args, **kwargs
        )
    ...
    async def get_player_status_history(self, *args, **kwargs):
        return await self.player_status_service.get_player_status_history(
            *args, **kwargs
        )
...
def create_auth_service(
    ...
    player_status_service = PlayerStatusService(
        identity_service, status_transition_service
    )
