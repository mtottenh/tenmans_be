...
from auth.schemas import AuthType, TokenResponse
...
class TokenService:
    ...
    def create_auth_tokens(self, player_id: str, auth_type: AuthType) -> TokenResponse:
        ...
        access_token = self.create_token(
            player_id, auth_type, self.config.access_token_expire
        )
        refresh_token = self.create_token(
            player_id, auth_type, self.config.refresh_token_expire, is_refresh=True
        )
        return TokenResponse(
            access_token=access_token, refresh_token=refresh_token, auth_type=auth_type
        )
