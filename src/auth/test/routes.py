...
async def register_test_user(
    user_data: PlayerEmailCreate, session: AsyncSession = Depends(get_session)
):
    ...
    try:
        ...
        if not system_user:
            raise HTTPException(
                ...
                detail="Unable to register new players. SYSTEM user not yet created",
            )
        player = await auth_service.create_player(
            user_data, actor=system_user, session=session
        )
...
async def login_test_user(
    login_data: PlayerLogin, session: AsyncSession = Depends(get_session)
):
