...
async def login_with_steam(
    ...
    if not system_user:
        raise HTTPException(
            ...
            detail="Unable to register/login players. SYSTEM user not yet created",
        )
    ...
    return_url = (
        str(request.base_url.replace(scheme="https")) + "api/v1/auth/steam/callback"
    )
    LOG.info(f"client_host: {request.client.host!s} base_url: {request.base_url!s}")
    return RedirectResponse(
        auth_request.redirectURL(
            return_to=return_url, realm=str(request.base_url.replace(scheme="https"))
        )
    )
...
async def steam_callback(
    ...
    try:
        ...
        if not player:
            ...
            system_user = await auth_service.get_player_by_name("SYSTEM", session)
            player = await auth_service.create_steam_player(
                steam_id, actor=system_user, session=session
            )
        ...
        state_id = await state_service.store_state(
            StateType.AUTH, tokens, metadata={"player_id": str(player.id)}
        )
        ...
        return RedirectResponse(redir_url, status_code=status.HTTP_303_SEE_OTHER)
...
async def exchange_state(
    state_id: str, state_service: StateService = Depends(get_state_service)
):
...
async def get_players(
    ...
    players = [
        p for p in players if p.name != "SYSTEM"
    ]  # Don't return the system user to external queries
    return players
...
async def get_players_current_season(
    _=Depends(access_token_bearer),
    session: AsyncSession = Depends(get_session),
    ...
    if season is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No active season"
        )
    players = await auth_service.get_all_players_with_basic_team_info(
        season.id, session
    )
    players = [
        p for p in players if p.name != "SYSTEM"
...
async def update_player(
    ...
    if player_id != str(player_details.id):
        raise HTTPException(
            ...
            detail=f"Players can only edit their own profiles {player_id} != {player_details.id!s} ",
        )
...
async def delete_player(
    player_id: str,
    session: AsyncSession = Depends(get_session),
    _player_details=Depends(get_current_player),
):
...
async def refresh_token(
    ...
    tokens = auth_service.create_auth_tokens(
        str(player.id), AuthType(token_payload["auth_type"])
    )
