...
class AvailabilityService:
    ...
    def _availability_audit_details(
        self, availability: PlayerAvailability, context: dict  # noqa: ARG002
    ) -> dict:
        ...
        return {
            ...
            "tournament_id": str(availability.tournament_id)
            if availability.tournament_id
            else None,
    ...
    async def add_availability(
        ...
        if player.id != actor.id:
            raise AvailabilityServiceError(
                "Players can only modify their own availability"
            )
    ...
    async def get_team_availability(
        ...
        stmt = select(Roster).where(
            Roster.team_id == team_id, Roster.status == RosterStatus.ACTIVE
        )
        result = await session.execute(stmt)
        ...
        for roster in roster_players:
            ...
            has_available = any(
                a.availability_type == AvailabilityType.AVAILABLE
                for a in player_availability
            )
            has_maybe = any(
                a.availability_type == AvailabilityType.MAYBE
                for a in player_availability
            )
            has_unavailable = any(
        ...
        if not team_availability:
            team_availability = TeamAvailability(
                team_id=team_id, tournament_id=tournament_id, date=date
            )
        ...
        team_availability.has_minimum_players = (
            len(available_players) >= 5
        )  # Assuming minimum 5 players
        team_availability.updated_at = datetime.now(timezone.utc)
    ...
    async def find_optimal_times(
        ...
        while current_date <= end_date.date():
            ...
            if (
                team_1_availability.has_minimum_players
                and team_2_availability.has_minimum_players
            ):
                # Create suggestion for this date
                suggested_time = datetime.combine(
                    current_date, datetime.min.time().replace(hour=19)
                )  # 7 PM
                ...
                suggestion = ScheduleSuggestion(
                    ...
                    confidence_score=self._calculate_confidence_score(
                        team_1_availability, team_2_availability
                    ),
    ...
    def _calculate_confidence_score(
        ...
        team_1_score = len(team_1_availability.available_players) / max(
            len(
                team_1_availability.available_players
                + team_1_availability.maybe_players
                + team_1_availability.unavailable_players
            ),
            1,
        )
        team_2_score = len(team_2_availability.available_players) / max(
            len(
                team_2_availability.available_players
                + team_2_availability.maybe_players
                + team_2_availability.unavailable_players
            ),
            1,
        )
    ...
    async def check_conflicts(
        self, fixture_id: uuid.UUID, proposed_time: datetime, session: AsyncSession
        ...
        team_1_availability = await self.get_team_availability(
            fixture.team_1, proposed_time.date(), fixture.tournament_id, session
        )
        ...
        if not team_1_availability.has_minimum_players:
            conflicts.append(
                ScheduleConflict(
                    ...
                    description="Team 1 does not have minimum players available",
                    severity="high",
        ...
        team_2_availability = await self.get_team_availability(
            fixture.team_2, proposed_time.date(), fixture.tournament_id, session
        )
        ...
        if not team_2_availability.has_minimum_players:
            conflicts.append(
                ScheduleConflict(
                    fixture_id=fixture_id,
                    conflict_type="insufficient_players",
                    description="Team 2 does not have minimum players available",
                    severity="high",
