...
class MatchService:
    ...
    def _result_audit_details(self, result: Result, context: dict) -> dict:  # noqa: ARG002
    ...
    async def _validate_team_captain(
        self, player: Player, team: Team, session: AsyncSession
    ) -> bool:
        """Validate that a player is captain of a team"""
        stmt = select(TeamCaptain).where(
            TeamCaptain.team_id == team.id, TeamCaptain.player_id == player.id
        )
    ...
    async def submit_result(
        ...
        is_team1_captain = await self._validate_team_captain(
            submitting_player, team_1, session
        )
        is_team2_captain = await self._validate_team_captain(
            submitting_player, team_2, session
        )
    ...
    async def confirm_result(
        ...
        submitting_team = (
            fixture.team_1 if result.submitted_by == fixture.team_1 else fixture.team_2
        )
        confirming_team = (
            fixture.team_2 if submitting_team == fixture.team_1 else fixture.team_1
        )
        is_captain = await self._validate_team_captain(
            confirming_player, confirming_team, session
        )

        if not is_captain:
            raise MatchServiceError(
                "Only the opposing team captain can confirm results"
            )
        ...
        if await self._check_all_maps_confirmed(fixture.id, session):
            await self.fixture_service.complete_fixture(
                fixture, confirming_player, session, audit_context
            )
    ...
    async def dispute_result(
        ...
        is_captain = await self._validate_team_captain(
            disputing_player, disputing_team, session
        )

        if not is_captain:
            raise MatchServiceError(
                "Only the opposing team captain can dispute results"
            )
    ...
    async def admin_override_result(
        ...
        if await self._check_all_maps_confirmed(fixture.id, session):
            await self.fixture_service.complete_fixture(
                fixture, admin, session, audit_context
            )
    ...
    async def _check_all_maps_confirmed(
        ...
        stmt = select(Result).where(Result.fixture_id == fixture_id)
        results = ((await session.execute(stmt)).scalars()).all()

        return all(
            r.confirmation_status == ConfirmationStatus.CONFIRMED for r in results
        )

    @AuditService.audited_transaction(
        action_type=AuditEventType.CREATE, entity_type="MatchPlayer"
    )
    ...
    async def get_match_players(
        ...
        stmt = select(MatchPlayer).where(MatchPlayer.fixture_id == fixture_id)
        result = (await session.execute(stmt)).scalars()
        return result.all()

    async def get_team_match_players(
        self, fixture_id: uuid.UUID, team_id: uuid.UUID, session: AsyncSession
    ) -> list[MatchPlayer]:
        """Get all players for a team in a match"""
        stmt = select(MatchPlayer).where(
            MatchPlayer.fixture_id == fixture_id, MatchPlayer.team_id == team_id
        )
        result = (await session.execute(stmt)).scalars()
        return result.all()
    ...
    async def get_match_results(
        ...
        stmt = (
            select(Result)
            .where(Result.fixture_id == fixture_id)
    ...
    async def get_match_summary(
        ...
        return {
            ...
            "has_disputes": any(
                r.confirmation_status == ConfirmationStatus.DISPUTED for r in results
            ),
        }
...
def create_match_service(
    ...
    fixture_svc: Optional[FixtureService] = None,
) -> MatchService:
