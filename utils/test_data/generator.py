...
from matches.models import ConfirmationStatus, Result
...
class TestDataGenerator:
    ...
    async def generate_teams(self, count: int):
        ...
        async with self.session.begin():
            for _ in range(count):
                ...
                team_captain = TeamCaptain(team_id=team_id, player_id=captain.id)
    ...
    async def generate_tournament(self):
        ...
        tournament = Tournament(
            ...
            scheduled_start_date=datetime.now(timezone.utc) - timedelta(days=15),
            scheduled_end_date=datetime.now(timezone.utc) + timedelta(days=15),
            format_config={
    ...
    async def generate_matches(self, tournament_id: uuid.UUID):
        ...
        for home_team in self.teams[: len(self.teams) // 2]:
            for away_team in self.teams[len(self.teams) // 2 :]:
                ...
                if fixture.scheduled_at < datetime.now(timezone.utc):
    ...
    async def _get_team_average_elo(self, team_id: uuid.UUID) -> float:
        ...
        # TODO: Implement actual team average ELO calculation using team_id
        _ = team_id  # Mark as used for now
...
async def main():
    ...
    async with AsyncSession(engine) as session:
        ...
        await generator.generate_all(num_players=50, num_teams=8, include_season=True)
