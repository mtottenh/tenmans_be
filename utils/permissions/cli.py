            ...
            try:
                ...
                if format == "csv":
                    PermissionReporter.generate_csv_report(
                        results, Path(output or "permission_audit.csv")
                    )
                elif format == "json":
                    PermissionReporter.generate_json_report(
                        results, Path(output or "permission_audit.json")
                    )
                else:
            ...
            try:
                ...
                if template:
                    ...
                    raise click.ClickException(
                        "Must specify either --permissions or --template"
                    )
                ...
                for perm_name in perm_names:
                    ...
                    if not perm:
                        raise click.ClickException(
                            f"Permission '{perm_name}' not found"
                        )
                    permission_ids.append(perm.id)

                role = await auth_service.create_role(
                    name, permission_ids, actor=system_user, session=session
                )
                click.echo(
                    f"Created role '{role.name}' with {len(role.permissions)} permissions"
                )
                await session.commit()
            except Exception as e:
                await session.rollback()
...
@click.argument("name")
@click.option(
    "--force/--no-force", default=False, help="Force deletion even if role is in use"
)
        ...
        async for session in get_session():
            try:
                ...
                if not force and role.players:
                    click.echo(
                        f"Role '{name}' is assigned to {len(role.players)} players."
                    )
                    click.echo("Use --force to delete anyway")
                    return

                await auth_service.delete_role(
                    role.id, actor=system_user, session=session
                )
    ...
    async def run():
        ...
        async for session in session_gen:
            ...
            for perm_name in sorted(all_permissions):
                try:
                    if not await auth_service.get_permission_by_name(
                        perm_name, session
                    ):
