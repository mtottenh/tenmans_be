...
from db.main import async_session, init_db
...
async def export_players_to_csv(output_file: str):
    ...
    async with async_session() as session:
        ...
        if not active_season:
            print(
                "WARNING: No active season found. Team information will be unavailable."
            )
        ...
        # Use asyncio.to_thread for blocking IO in async context
        await asyncio.to_thread(_write_csv, players, active_season_id, output_file)

        print(f"Export complete! {len(players)} players written to {output_file}")

    print("Database connection closed.")


def _write_csv(players, active_season_id, output_file):
    """Synchronous CSV writing function to be run in thread"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", default="players.csv", help="Output CSV file (default: players.csv)"
    )
