    ...
    try:
        ...
        if premier_section:
            ...
            if current_rank and best_rank:
                ...
                current_elo = (
                    current_rank.get_text(strip=True).replace(",", "").replace("-", "0")
                )
                best_elo = (
                    best_rank.get_text(strip=True).replace(",", "").replace("-", "0")
                )
                return {
                    "player_id": player_id,
        ...
        # Use thread for blocking IO in async context
        await asyncio.to_thread(_write_error_response, player_id, response.text)
        print(f"{player_id}: Unable to scrape rank")
        return None
        # raise ParseException(f"Rank information not found")
    except Exception as e:
        raise ScrapeError(f"An error occurred while fetching player data {e}") from e


def _write_error_response(player_id: str, response_text: str):
    """Helper to write error response file synchronously"""
    with Path(f"{player_id}.response.html").open("w", encoding="utf-8") as f:
            ...
            for p in players:
                ...
                try:
                    ...
                    for elo_type in ["current_elo", "highest_elo"]:
                        if elo_type in player_rank and int(
                            player_rank[elo_type]
                        ) != getattr(p, elo_type):
                    ...
                    print(
                        f"{p.name} - {p.steam_id} - {p.current_elo} - {p.highest_elo}"
                    )
                except Exception as e:
                    print(f"{e}")
                    break
