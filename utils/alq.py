...
def parse_date(ctx, param, date_str: Optional[str]) -> Optional[datetime]:
    ...
    if not date_str:
        return None
    try:
        # Parse as UTC datetime by appending timezone
...
@click.option(
    "--entity-type", default=None, help="Filter by entity type (e.g., Player, Team)"
)
...
@click.option(
    "--start-date", default=None, help="Start date (YYYY-MM-DD)", callback=parse_date
)
@click.option(
    "--end-date", default=None, help="End date (YYYY-MM-DD)", callback=parse_date
)
...
async def list(
    ...
    async for session in get_session():
        ...
        click.echo(
            f"\nFound {result.total_count} events" + f" (showing {len(result.events)})"
        )
...
async def summary(days, entity_type):
    ...
    async for session in get_session():
        stats = await audit_service.get_summary_statistics(
            session=session, days=days, entity_type=entity_type
        )
        ...
        for entity_type_name, count in stats["entity_counts"].items():
            click.echo(f"  {entity_type_name}: {count}")
...
@click.option(
    "--include-cascaded/--no-cascaded", default=True, help="Include cascaded events"
)
